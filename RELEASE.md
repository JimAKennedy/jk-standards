...
python -c "import jk_standards; assert jk_standards.__version__ == '0.6.0'"
```

Version lives in exactly two source-of-truth sites and must agree:

- `pyproject.toml` → `version = "0.6.0"`
- `src/jk_standards/__init__.py` → `__version__ = "0.6.0"`
...
`rev:` in their `.pre-commit-config.yaml` and the `@v0.6.0` references in the

...
| ship | Refuses to ship an unfinished milestone, rebases onto the default branch when the base has moved (re-running the gates on the rebased head; conflicts stop the run), syncs the changelog and roadmap, pushes, opens a pull request whose body is generated from the ledger and from git trailers rather than written from memory, then watches the checks and merges on green — the human review gate sits before this command, in the milestone report — prompting for `/jk:close` afterwards. A failed check or a requested change stops the run rather than being fixed in place |
